...
OUT = Path("artifacts/commission_series_round_002.json")
...
def float_in_enclosure(x: float, lo: Fraction, hi: Fraction) -> bool:
    q = Fraction(*x.as_integer_ratio())
    return lo <= q <= hi
...
def main() -> None:
    ...
    series_contains_lib = float_in_enclosure(lib_exp_m1, lo, hi)
    ...
    payload = {
        "metrics": {
            ...
            "series_contains_lib_exp": series_contains_lib,
        ...
        "reference": {
            ...
            "source": (
                ...
                "2**10; math.exp(0)==1; Decimal(-1).exp(); Leibniz next-term "
                "remainder for the alternating series of exp(-1). Sequential "
                "0.9 multiplies compared to pow with an explicit 16-ulp "
                "binary64 rounding tolerance."
            ),
            "two_expected": 1024.0,
        },
    }
