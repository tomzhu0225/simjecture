...
for command_name in python3 bwrap; do
...
"$package_root/.venv/bin/python" -m pip install "$package_root/packages/simjecture-${release_version}-py3-none-any.whl[tui]"
if [[ "${1:-}" == "--with-dsh" ]]; then
  for command_name in node npm; do
    command -v "$command_name" >/dev/null || { echo "Missing: $command_name" >&2; exit 1; }
  done
  node -e 'const [major, minor] = process.versions.node.split(".").map(Number); if (!((major === 22 && minor >= 19) || major >= 24)) { console.error("DSH requires Node 22.19+ or 24+"); process.exit(1); }'
...
printf 'Setup complete. Log in with your chosen native agent CLI, then run ./launch.sh\n'
