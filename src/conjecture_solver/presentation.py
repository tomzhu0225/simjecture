"""Shared human-facing projections of the durable typed claim ledger.

This module deliberately has no terminal or web framework dependency. Every
human interface should use these projections so a scientific claim never moves
between the hypothesis tree and validation ledger merely because the operator
changed clients.
"""
...
def is_scientific_claim(claim: ClaimSummary) -> bool:
    """Return whether a durable claim belongs in the hypothesis tree."""

    return claim.kind == SCIENTIFIC_KIND or (claim.id == "claim_root" and claim.relation == "root")
...
def build_hypothesis_tree(claims: Iterable[ClaimSummary]) -> tuple[ClaimTreeRow, ...]:
    ...
    ordered = tuple(claim for claim in claims if is_scientific_claim(claim))
    ...
    roots.extend(
        claim
        for claim in ordered
        if claim.id != "claim_root" and (claim.relation == "root" or claim.parent_id not in by_id)
    )
...
def scientific_ancestor_id(
    ...
    while current is not None and current.id not in visited:
        ...
        if is_scientific_claim(current):
            return current.id
...
def build_validation_tree(
    ...
    to a child hypothesis remain with that child rather than its parent.
    """
    ...
    candidates = tuple(
        claim
        for claim in ordered
        if not is_scientific_claim(claim)
